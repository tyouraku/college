...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        identity = x.detach().clone() if self.use_residual else x  # 关键修改2
        out = self.conv(x)
        out = self.bn(out)
        out = self.relu(out)
        ...
        if self.use_residual:
            if identity.shape != out.shape:
                ...
                identity = F.pad(identity.clone(), (0,0,0,0,0,out.shape[1]-identity.shape[1]))  # 关键修改3
            out = out + identity
...
class Classifier(nn.Module):
    def __init__(
        ...
        # input image with size [batch_size, in_channels, H, W]
        # Network structure:
        #             kernel_size  stride  padding  out_channels  use_residual
        # ConvBlock       7          2        3          64         False     # initial conv
        # maxpool         2          2        0                               # Initial downsampling
        # ConvBlock       3          1        1          64         True      # Residual block
        # ConvBlock       3          2        1          128        False      # Downsample
        # ConvBlock       3          1        1          128        True      # Residual block
        # ConvBlock       3          2        1          256        False      # Downsample
        # dropout(p), where p is input parameter of dropout ratio

        self.conv_net = nn.Sequential(
            ConvBlock(in_channels, 64, 7, 2, 3, use_batch_norm, False),
            nn.MaxPool2d(2, 2),
            ConvBlock(64, 64, 3, 1, 1, use_batch_norm, True),
            # ConvBlock(64, 64, 3, 1, 1, use_batch_norm, True), ConvBlock的数量为6
            # ConvBlock(64, 64, 3, 1, 1, use_batch_norm, True), ConvBlock的数量为7
            ConvBlock(64, 128, 3, 2, 1, use_batch_norm, False),
            ConvBlock(128, 128, 3, 1, 1, use_batch_norm, True),
            ConvBlock(128, 256, 3, 2, 1, use_batch_norm, False),
...
class STN(nn.Module):
    def __init__(self, in_channels):
        ...
        self.localization_conv = nn.Sequential(
            ConvBlock(in_channels, 32, 3, 2, 1, True),
            ConvBlock(32, 64, 3, 2, 1, True),
            ConvBlock(64, 128, 3, 2, 1, True),
        ...
        # The output of the last linear layer should be 6, which represents the parameters of affine transformation.
        ...
        # achieve this goal?
        # The identity transformation matrix should be like:
        # [[1, 0, 0],
        #  [0, 1, 0]]
