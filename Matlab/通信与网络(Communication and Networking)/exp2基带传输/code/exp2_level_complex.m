...
for cnt1 = 1:length(M)
    ...
    % 理论平均电平能量
    if M(cnt1)==4
        Es_theory = 1; % PSK调制：Es=A²
    elseif M(cnt1)==16
        Es_theory = 2*(M(cnt1)-1)/3; % QAM调制：Es=2(M-1)/3A²
    end

    for cnt2 = 1:length(SNR)
        % 加性噪声
        noise_power = Es_theory/SNR(cnt2); % 选择(b)星座图的理论符号能量
        noise = sqrt(noise_power) * (randn(size(data_mod)) + 1j*randn(size(data_mod))); % 复数噪声实部和虚部的方差均为σ²

        % 加性高斯噪声信道，获得接收信号（data_rx）
        data_rx = data_mod + noise;
        if M_cur==16 && ismember(SNR_dB(cnt2),SNR_dB_rec)
            figure;
            scatter(real(data_rx(1:1000)), imag(data_rx(1:1000)), 10, 'filled'); % 在复平面上绘制接收信号的星座图
            box on; grid on;
...
fprintf('M=4, SNR=2dB时的符号差错个数: %d\n',SER(1, 8));
fprintf('M=4, SNR=2dB时的比特差错个数: %d\n',BER(1, 8));

fprintf('M=4, SNR=5dB时的符号差错个数: %d\n',SER(1, 11));
fprintf('M=4, SNR=5dB时的比特差错个数: %d\n',BER(1, 11));

fprintf('M=4, SNR=8dB时的符号差错个数: %d\n',SER(1, 14));
fprintf('M=4, SNR=8dB时的比特差错个数: %d\n',BER(1, 14));

fprintf('M=16, SNR=8dB时的符号差错个数: %d\n',SER(2, 14));
fprintf('M=16, SNR=8dB时的比特差错个数: %d\n',BER(2, 14));

fprintf('M=16, SNR=11dB时的符号差错个数: %d\n',SER(2, 17));
fprintf('M=16, SNR=11dB时的比特差错个数: %d\n',BER(2, 17));

fprintf('M=16, SNR=14dB时的符号差错个数: %d\n',SER(2, 20));
fprintf('M=16, SNR=14dB时的比特差错个数: %d\n',BER(2, 20));
...
% 理论值
ser_theory = zeros(length(M), length(SNR));
for cnt1 = 1:length(M)
    M_cur = M(cnt1);
    if M_cur == 4 % PSK
        ser_theory(cnt1,:) = 2*(1-erf(sin(pi/M(cnt1))*sqrt(SNR/2)))/2;
    elseif M_cur == 16 % QAM
        ser_theory(cnt1,:) = 4*(1-1./sqrt(M(cnt1)))*(1-erf(sqrt(3./(2*(M(cnt1)-1))*SNR/2)))/2;
    end
end
