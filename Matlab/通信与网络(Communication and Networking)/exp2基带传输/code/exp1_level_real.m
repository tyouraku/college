...
for cnt1 = 1:length(M)
    ...
    for cnt2 = 1:length(SNR)
        ...
        noise = sqrt(noise_power) * randn(size(data_mod)); % 生成服从对应方差高斯分布的噪声信号
        ...
        elseif M_cur == 4
            ...
            SER(cnt1,cnt2) = sum(bit_err(1,:) | bit_err(2,:)); % 2个比特中任意一个出错都是符号出错
        end
    end
end

fprintf('M=2, SNR=2dB时的符号差错个数: %d\n',SER(1, 8));
fprintf('M=2, SNR=2dB时的比特差错个数: %d\n',BER(1, 8));

fprintf('M=2, SNR=5dB时的符号差错个数: %d\n',SER(1, 11));
fprintf('M=2, SNR=5dB时的比特差错个数: %d\n',BER(1, 11));

fprintf('M=2, SNR=8dB时的符号差错个数: %d\n',SER(1, 14));
fprintf('M=2, SNR=8dB时的比特差错个数: %d\n',BER(1, 14));

fprintf('M=4, SNR=8dB时的符号差错个数: %d\n',SER(2, 14));
fprintf('M=4, SNR=8dB时的比特差错个数: %d\n',BER(2, 14));

fprintf('M=4, SNR=11dB时的符号差错个数: %d\n',SER(2, 17));
fprintf('M=4, SNR=11dB时的比特差错个数: %d\n',BER(2, 17));

fprintf('M=4, SNR=14dB时的符号差错个数: %d\n',SER(2, 20));
fprintf('M=4, SNR=14dB时的比特差错个数: %d\n',BER(2, 20));
...
% 理论值
SER_theory = (2-2./M).*(1-erf(sqrt(3./(M.^2-1)*SNR/2)))/2;
...
elseif M==4
    ...
    for n = 1:(N/2)
        current_level = data_rx(n);
        if current_level < -2
            data_bit_demod(2*n-1 : 2*n) = [0, 0]; % (-∞, -2): 00
        elseif current_level < 0
            data_bit_demod(2*n-1 : 2*n) = [0, 1]; % [-2, 0): 01
        elseif current_level < 2
            data_bit_demod(2*n-1 : 2*n) = [1, 1]; % [0, 2): 11
        else
            data_bit_demod(2*n-1 : 2*n) = [1, 0]; % [2, +∞): 10
        end
    end
end
end