...
% 设置系统参数
N = 100;% 用户数
lambdas = 0.1:0.5:50;% 单个用户的帧到达率集合
b = 100;
R = 1e5;
T_fr = b/R;
...
function SuccessFrameNum_Users = SuccessFrameNumCalcu(TotalFrameArrivalConditions,T_fr,N)
    ...
    TotalFrameOwners = TotalFrameArrivalConditions(2,:); % 修正此处的索引
    TotalFrameNum = length(TotalFrameArrivalTimes);
    TransmissionConditions = zeros(1, TotalFrameNum);
    
    for i = 1:TotalFrameNum
        % 检查前一帧：如果是第一帧，无前一帧冲突
        pre_ok = (i == 1) || (TotalFrameArrivalTimes(i) >= TotalFrameArrivalTimes(i-1) + T_fr);
        % 检查后一帧：如果是最后一帧，无后一帧冲突
        next_ok = (i == TotalFrameNum) || (TotalFrameArrivalTimes(i+1) >= TotalFrameArrivalTimes(i) + T_fr);
        if pre_ok && next_ok
    ...
    for n = 1:N
        SuccessFrameNum_Users(n) = sum(TotalFrameOwners(logical(TransmissionConditions)) == n);
    end
