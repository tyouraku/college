...
%% ====================== 匹配滤波器 ======================
h1_t = sqrt(2/T) * cos(2*pi*f_c*(0:delta_t:T-delta_t)); % 方法1
h2_t = (1/sqrt(T)) * ones(1, num);                      % 方法2
...
for idx = 1:length(N0_list)
    ...
    ser_theory(idx) = (2*(M-1)/M) * qfunc(sqrt(6*EsN0(idx)/(M^2-1)));
...
for n = 1:N
    ...
    if current_level < -2
       demod_bit_data(2*n-1:2*n) = [0, 0]; % -3A -> 00
    elseif current_level < 0
        demod_bit_data(2*n-1:2*n) = [0, 1]; % -A -> 01
    elseif current_level < 2
        demod_bit_data(2*n-1:2*n) = [1, 1]; % A -> 11
    else
        demod_bit_data(2*n-1:2*n) = [1, 0]; % 3A -> 10
    end
end
end
