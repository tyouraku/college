...
% 虽不是TODO范围，但做了修改，改成二维向量匹配后面的双重循环
BER_sim1 = zeros(length(EbN0_dB),3);   % 方法1仿真BER
BER_sim2 = zeros(length(EbN0_dB),3);   % 方法2仿真BER
...
for idx = 1:length(N0_list)
    ...
    % 遍历采样偏差
     for j = 1:length(offset_list)
        ...
        bit_err1 = x_hat1 ~= bit_data(1:length(new_ind));
        bit_err2 = x_hat2 ~= bit_data(1:length(new_ind));
...
BER_theory = qfunc(sqrt(2*EsN0));  % BPSK理论BER
...
for n = 1:N
    current_level = rx_data(n);
     if current_level < 0
        demod_bit_data(n) = 0; % 负电平判决为0
     else
        demod_bit_data(n) = 1; % 正电平判决为1
     end
end
end
%时间对齐的卷积函数
